    ...
    public partial class TextMeshPro
    {
        ...
        protected virtual void GenerateTextMesh()
        {
            ...
            for (int i = 0; i < m_TextProcessingArray.Length && m_TextProcessingArray[i].unicode != 0; i++)
            {
                ...
                // Handle potential character substitutions 处理潜在的字符替换
                ...
                // When using Linked text, mark character as ignored and skip to next character. 使用链接文本时，将字符标记为忽略并跳到下一个字符。 
                #region Linked Text
                ...
                // Handle Font Styles like LowerCase, UpperCase and SmallCaps. 处理字体样式，如小写，大写和小写。 
                #region Handling of LowerCase, UpperCase and SmallCaps Font Styles
                ...
                // Look up Character Data from Dictionary and cache it. 从字典中查找字符数据并缓存它。 
                ...
                // Cache glyph metrics 缓存字形指标
                GlyphMetrics currentGlyphMetrics = m_cached_TextElement.m_Glyph.metrics;

                // Optimization to avoid calling this more than once per character. 优化以避免对每个字符调用多次此函数。 
                bool isWhiteSpace = charCode <= 0xFFFF && char.IsWhiteSpace((char)charCode);

                // Handle Kerning if Enabled. 处理字距，如果启用。
                ...
                // Initial Implementation for RTL support. RTL支持的初始实现。
                ...
                // Set Padding based on selected font style 根据选定的字体样式设置填充
                ...
                // Determine the position of the vertices of the Character or Sprite. 确定角色或精灵的顶点位置。 
                ...
                // Check if we need to Shear the rectangles for Italic styles 检查我们是否需要剪切矩形为斜体样式 
                ...
                // Handle Character Rotation 处理字符旋转
                ...
                // Set Characters to not visible by default. 默认情况下，将字符设置为不可见。
                m_textInfo.characterInfo[m_characterCount].isVisible = false;
                ...
                // Setup Mesh for visible text elements. ie. not a SPACE / LINEFEED / CARRIAGE RETURN. 设置可见文本元素的网格。
                ...
                if (charCode == 9 || (isWhiteSpace == false && charCode != 0x200B && charCode != 0xAD && charCode != 0x03) || (charCode == 0xAD && isSoftHyphenIgnored == false) || m_textElementType == TMP_TextElementType.Sprite)
                {
                    ...
                    // Injected characters do not override margins 注入的字符不会重写页边距
                ...
                // Tracking of potential insertion positions for Ellipsis character 跟踪省略号字符的潜在插入位置 
                ...
                // Store Rectangle positions for each Character. 存储每个字符的矩形位置。
                ...
                // Handle xAdvance & Tabulation Stops. Tab stops at every 25% of Font Size. 处理xAdvance和制表停止。 标签在字体大小的25%处停止。 
                ...
                // Handle Line Spacing Adjustments + Word Wrapping & special case for last line. 处理行间距调整+字包装和特殊情况的最后一行。 
                ...
                // Save State of Mesh Creation for handling of Word Wrapping 保存网格创建状态，以处理Word换行 
            ...
            // Check Auto Sizing and increase font size to fill text container.  勾选“自动调整大小”并增加字体大小以填充文本容器。 
            ...
            // If there are no visible characters or only character is End of Text (0x03)... no need to continue 如果没有可见字符或只有字符是文本结束(0x03)… 不需要继续 
            ...
            // Partial clear of the vertices array to mark unused vertices as degenerate. 部分清除顶点数组，将未使用的顶点标记为退化顶点。 
